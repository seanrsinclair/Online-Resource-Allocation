...
def weighted_geometric_mean(x, w):
    return np.exp(np.dot(w, np.log(x)) / np.sum(w))


def get_individual_utility(weights, allocation, utility, epsilon = None):
    if utility == 'linear':
        ...
        return np.min(allocation / weights) + epsilon * np.sum(allocation)
    elif utility == 'cobb_douglas':
        return weighted_geometric_mean(allocation,weights)
    elif utility == 'cobb_douglas_epsilon':
        return weighted_geometric_mean(allocation,weights) + epsilon*np.sum(allocation)




def get_utility(weights, allocation, utility):
    utils = []
    for typ in range(weights.shape[0]):
        utils.append(get_individual_utility(weights[typ], allocation[typ], utility))
    return utils
...
def get_proportionality(weights, allocation, sizes, budget, utility):
    proportionality = 0
    equal_allocation = budget / np.sum(sizes)
    for t in range(sizes.shape[0]):
        for typ in range(sizes.shape[1]):
            if get_individual_utility(weights[typ], allocation[t,typ], utility) < get_individual_utility(weights[typ], equal_allocation, utility):
                val = get_individual_utility(weights[typ], equal_allocation, utility) - get_individual_utility(weights[typ], allocation[t,typ], utility)
                proportionality = max(proportionality, val)
    return proportionality


def get_envy(weights, allocation, sizes, budget, utility):
    envy = 0
        ...
        for typ_1 in range(sizes.shape[1]):
            for t_2 in range(sizes.shape[0]):
                for typ_2 in range(sizes.shape[1]):
                    if get_individual_utility(weights[typ_1], allocation[t_1,typ_1], utility) < get_individual_utility(weights[typ_1], allocation[t_2, typ_2], utility):
                        val = get_individual_utility(weights[typ_1], allocation[t_2, typ_2], utility) - get_individual_utility(weights[typ_1], allocation[t_1, typ_1], utility)
                        envy = max(envy, val)
    return envy



def get_counterfactual_envy(weights, allocation, sizes, budget, utility, opt):
    envy = 0
    for t in range(sizes.shape[0]):
        for typ in range(sizes.shape[1]):
            val = get_individual_utility(weights[typ], allocation[t, typ], utility) - get_individual_utility(weights[typ], opt[t, typ], utility)
...
def verify_fairness(weights, allocation, sizes, budget, utility, opt):
    ...
    envy = get_envy(weights, allocation, sizes, budget, utility)
#     envy = 0
    proportionality = get_proportionality(weights, allocation, sizes, budget, utility)
    c_envy = get_counterfactual_envy(weights, allocation, sizes, budget, utility, opt)
    return waste, envy, proportionality, c_envy
...
def generate_cvxpy_solve(num_types, num_resources, utility):
    if utility == 'linear':
        ...
        return generate_cvxpy_solve_cobb_douglas(num_types, num_resources)
    elif utility == 'cobb_douglas_epsilon':
        return generate_cvxpy_solve_cobb_douglas_epsilon(num_types, num_resources)
...
def generate_cvxpy_solve_leontief_epsilon(num_types, num_resources):
    ...
    def solver(true_sizes, true_weights, true_budget, epsilon):
        ...
        weights.value = true_weights
        budget.value = true_budget
        objective = cp.Maximize(cp.log(cp.min(cp.multiply(x, 1 / weights), axis=1) + epsilon * cp.sum(x, axis=1)) @ sizes)
...
def generate_cvxpy_solve_cobb_douglas(num_types, num_resources):
#     print(f'New Solver.  Num Types: {num_types}, Num Resources: {num_resources}')
    x = cp.Variable(shape=(num_types,num_resources))
    ...
    # objective = cp.Maximize(cp.geo_mean(x[0], [.2, .5]))
    ...
    def solver(true_sizes, true_weights, true_budget):
        ...
        weights.value = true_weights
        budget.value = true_budget
        objective = cp.Maximize(cp.sum([true_sizes[i]*cp.log(cp.geo_mean(x[i,:], true_weights[i,:])) for i in range(num_types)]))
...
def generate_cvxpy_solve_cobb_douglas_epsilon(num_types, num_resources):
    # print(f'New Solver.  Num Types: {num_types}, Num Resources: {num_resources}')
    x = cp.Variable(shape=(num_types,num_resources))
    ...
    def solver(true_sizes, true_weights, true_budget, epsilon):
        ...
        weights.value = true_weights
        budget.value = true_budget
        objective = cp.Maximize(cp.sum([sizes[i]*cp.log(cp.geo_mean(x[i], true_weights[i]) + epsilon * cp.sum(x[i])) for i in range(num_types)]))
        prob = cp.Problem(objective, constraints)
